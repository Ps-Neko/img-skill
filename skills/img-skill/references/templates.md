...
이 템플릿은 복사된 프롬프트나 고정된 시각 프리셋이 아니라, 사용자가 채우는 작성 뼈대다. 먼저 사용자의 보존 조건과 목적을 채우고, 적용되지 않는 항목은 생략한다. 특히 글자가 필요하지 않으면 타이포그래피를 억지로 넣지 않는다. 출력 전에는 [quality-check.md](quality-check.md)를 실행한다.

- **Account-observed pattern / 계정 관찰 패턴:** 공개 게시물 연구에서 보존 우선, 관계형 구성, 제작 동작, 역할·면적 기반 색, 필요한 경우의 구성적 타이포그래피, 예상 실패에 한정한 제한이 관찰되었다. 이는 계정 소유자의 의도나 공식 방법에 대한 주장이 아니다.
...
목적/사용처: [purpose/use]
주제: [subject]
반드시 유지: [preservation requirements]
...
3. Preservation — [preservation requirements]를 바꾸지 않는다.
...
## Poster Template — strong evidence, 28/54

**Account-observed pattern / 계정 관찰 패턴 — strong evidence, 28/54:** 포스터 관련 표본에서 반복 관찰된 설계 요소를 바탕으로 한 뼈대이며, 원문 프롬프트나 공식 방법의 복제가 아니다.

```text
[purpose/use]용 포스터. 주제는 [subject]이며 [preservation requirements]를 유지한다.
...
```text
[purpose/use]용 제품 광고. 제공된 [subject]의 [preservation requirements: silhouette, proportions, material, color placement, stitching, sole, supplied logo only]를 스타일보다 먼저 정확히 보호한다.
...
```text
[purpose/use]용 인물/패션 이미지. [subject]에서 [preservation requirements]를 유지한다.
...
```text
[purpose/use]용 건축/공간 이미지. [subject]의 [preservation requirements]를 유지한다.
...
```text
[purpose/use]용 3D 렌더 또는 일러스트레이션. 사용자가 선택한 [subject]의 [preservation requirements]를 유지한다.
